...
def test_transform_with_bad_args():
    ...
    with pytest.raises(TypeError) as e:
        transform(datapoint)
    assert str(e.value) == "'<=' not supported between instances of 'datetime.date' and 'str'"


def test_get_cbr_er(mocker):
    start_str = '1998-12-05'
    end_str = '1998-12-08'
    mocker.patch('parsers.getter.cbr_fx.make_url', mock.MagicMock(return_value='http://www.cbr.ru/scripts/XML_dynamic.asp?date_req1=05/12/1998&date_req2=08/12/1998&VAL_NM_RQ=R01235'))
    result = [
        {'date': start_str, 'freq': 'd', 'name': 'USDRUR_CB', 'value': Decimal('19.5700')},
        {'date': end_str, 'freq': 'd', 'name': 'USDRUR_CB', 'value': Decimal('20.4000')}
    ]
